...
import kotlinx.android.synthetic.main.fragment_partner_preference.build_bar
import kotlinx.android.synthetic.main.fragment_partner_preference.caste_group
import kotlinx.android.synthetic.main.fragment_partner_preference.living_arrangement_group
import org.jitsi.meet.sdk.JitsiMeetActivityDelegate.onBackPressed
import kotlin.collections.ArrayList


class PartnerPreferenceFragment : Fragment(),
    OnLanguageSelect,
OnPrefChange, EditInformation {
    ...
    val profession by lazy {   ArrayList<String>()}
    ...
    override fun onCreateView(
        ...
        return LayoutInflater.from(context!!).inflate(R.layout.fragment_partner_preference,container,false)
    }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        onFragmentChange?.selectedPos(4)
        nationalityDialog = CountryPickerDialog(context!!)
        prefCancel.setOnClickListener { onBackPressed() }
        originDialog = CountryPickerDialog(context!!)
        if (get(PrefKeys.INSTANCE.isDataSaved, false) == true) {
            getValues()
        }
    ...
    private fun setValues() {
        pref_complexion.text = get(PrefKeys.INSTANCE.preF_COLORCOMPLEXION, "")
        pref_education.text = get(PrefKeys.INSTANCE.preF_EDUCATION, "")
        pref_eyes_color.text = get(PrefKeys.INSTANCE.preF_EYECOLOR, "")
        pref_hair_color.text = get(PrefKeys.INSTANCE.preF_HAIRCOLOR, "")
    ...
    private fun setOnClick() {
        ...
        preference_done.setOnClickListener {
            ...
            setprefObject(PrefKeys.INSTANCE.userprefs, userPreference)
            setPref(PrefKeys.INSTANCE.isDataSaved, true)
            onBackPressed()
        }
...
private fun setMarriagePlansClick() {
       ...
       // marriage_plan.adapter=PrefAdapter(arraylist, context!!, PrefType.MARRIAGEPLANS,  userPreference?.marriagePlan?: arrayListOf(),this)
    }
  ...
  private fun setAfterMarriageArrangementsClick() {
        ...
        //life_after_marriage.adapter=PrefAdapter(arraylist, context!!, PrefType.LIVINGARRANGEMENTSAFTERMARRIAGE, userPreference?.livingArrangementAfterMarriage?: arrayListOf(),this)
    }
 ...
 private fun setPrayClick() {
        ...
        //prays_group.adapter=PrefAdapter(arraylist, context!!, PrefType.PRAYS,userPreference?.prays?: arrayListOf(),this)
    }
    ...
    private fun setMaritalStatusClick() {
        ...
        marital_status_recycler.adapter=PrefAdapter(arraylist, context!!, PrefType.MARITALSTATUS,userPreference?.marital?: arrayListOf<String>(),this)
    }

    private fun setCasteClick() {
        val arraylist= arrayListOf("Sunni","Shia","Other")
        caste_group.adapter=PrefAdapter(arraylist,context!!,PrefType.SECT,userPreference?.sect?: arrayListOf<String>(),this)
    }

    private fun setLivingArrangementClick() {
        val arraylist= arrayListOf("Living Alone","Living with Family","Living with Friends","Other")
        living_arrangement_group.adapter=PrefAdapter(arraylist,context!!,PrefType.CURRENTLIVINGARRANGEMENT,userPreference?.livingArrangement?: arrayListOf<String>(),this)
    }

    private fun setBuildClick() {
        val arraylist= arrayListOf("Athletic","Large","Medium","Slim")
        build_bar.adapter=PrefAdapter(arraylist,context!!,PrefType.BUILD,userPreference?.build?: arrayListOf<String>(),this)
    }

  private fun setQuranClick() {
        val arraylist= arrayListOf("Yes","No")
      //  quran_radio_group.adapter=PrefAdapter(arraylist,context!!,PrefType.QURAN,userPreference?.completedQuran?: arrayListOf(),this)
    }
    ...
    private fun setHalalClick() {
        ...
        //halal_radio_group.adapter=PrefAdapter(arraylist,context!!,PrefType.HALAL,userPreference?.halal?: arrayListOf(),this)

    }

    private fun setRevertClick(){
        val arraylist= arrayListOf("Yes","No")
        ///revert_group.adapter=PrefAdapter(arraylist,context!!,PrefType.REVERT,userPreference?.halal?: arrayListOf(),this)
    }

    private fun setBeardClick(){
        val arraylist= arrayListOf("Yes","No")
        //beard_group.adapter=PrefAdapter(arraylist,context!!,PrefType.BEARD,userPreference?.beard?: arrayListOf(),this)
    }

    private fun setWearClick() {
        val arraylist= arrayListOf("Hijab","Naqab")
        //wear_group.adapter=PrefAdapter(arraylist,context!!,PrefType.WEAR,userPreference?.wears?: arrayListOf(),this)
    }
    ...
    private fun openDialog(item: String,textView: TextView) {
        ...
        val dialog=ProfileItemFragment().apply {
            infoSelectListener(this@PartnerPreferenceFragment,textView)
        }
    ...
    private fun openLanguageDialog() {
        val dialog = LanguageDialog().apply {
          //  languageSelectListener(this@PartnerPreferenceFragment)
        }
